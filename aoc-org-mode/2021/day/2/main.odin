...
interpret :: proc ( file_path_name : string, map_simbols : ^map[string]string ) ->
    (string, ^[]u8, bool) {
...
second_part :: proc () -> bool {
	...
	if strings.contains (atoms[0], map_simbols["forward"]) {
	    ...
	    if DEBUG_STUFF {
		fmt.printf ("forward %v :: horizontal %v, depth %v, aim %v\n", digite, horizontal, depth, aim)
	    }
	} else if strings.contains (atoms[0], map_simbols["up"]) {
	    // depth -= digite
	    aim -= digite
	    ...
	    if DEBUG_STUFF {
		fmt.printf ("up %v :: horizontal %v, depth %v, aim %v\n", digite, horizontal, depth, aim)
	    }
	} else if strings.contains (atoms[0], map_simbols["down"]) {
	    aim += digite
	    // depth += digite
	    if DEBUG_STUFF {
		fmt.printf ("down %v :: horizontal %v, depth %v, aim %v\n", digite, horizontal, depth, aim)
	    }
	}
