...
reason_about_gama_rate_one_bit_be_one :: proc(
  arrBits: map[int]string,
  idx: int,
) -> bool {
...
reason_about_epsilon_rate_get_binary :: proc(
  arrData: map[int]string,
) -> map[int]string {
  return reason_about_gama_rate_get_binary_digit(arrData, true)
}

filter_report_diagnostic_rating_oxygen_co2 :: proc(
  arrData: map[int]string,
  size_td: int,
...
reason_about_gama_rate_get_binary_digit :: proc(
  arrData: map[int]string,
  flag_invert: bool = false,
) -> map[int]string {
...
first_part :: proc() {
  ...
  map_string_of_bits_gama_rate: map[int]string =
    reason_about_gama_rate_get_binary_digit(strings_values)
  defer delete(map_string_of_bits_gama_rate)

  map_string_of_bits_epsilon_rate: map[int]string =
    reason_about_epsilon_rate_get_binary(strings_values)
  ...
  epsilon_rate_str: string = mapstring_to_string(
    map_string_of_bits_epsilon_rate,
  )
...
loop_to_filter_map_string_oxygen_co2 :: proc(
  lenstringvalues: int,
  filtered_map_strings_proc: map[int]string,
  is_co2_check: bool = false,
) -> map[int]string {

  filtered_map_strings: map[int]string = filtered_map_strings_proc

  loop_colum_digit: for colum_idx in 0 ..< lenstringvalues {
    filtered_map_strings = filter_report_diagnostic_rating_oxygen_co2(
      filtered_map_strings,
      colum_idx,
      is_co2_check,
    )
...
loop_to_filter_map_string_co2 :: proc(
  len_strings: int,
  filtered_map_strings: map[int]string,
) -> map[int]string {

  return loop_to_filter_map_string_oxygen_co2(
    len_strings,
    filtered_map_strings,
    true,
  )
...
second_part :: proc() {
  ...
  filtered_oxygen_map_strings = loop_to_filter_map_string_oxygen_co2(
    len_strings,
    filtered_map_strings,
  )

  filtered_co2_map_strings = loop_to_filter_map_string_co2(
    len_strings,
    filtered_map_strings,
  )

  oxygen_generator_num: int = string_binary_to_binary_integer_decimal(
    filtered_oxygen_map_strings[0],
  )

  co2_rate_num: int = string_binary_to_binary_integer_decimal(
    filtered_co2_map_strings[0],
  )
